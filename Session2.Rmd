...
```{r}
  ggplot(data = lines_HARV) +
  geom_sf( aes(color = TYPE, size = TYPE)) + 
  scale_color_manual(values = road_colors) +
  labs(color = 'Road Type') +
...
```{r}
ggplot(data = lines_HARV) + 
  geom_sf( aes(color = TYPE), size = 1.5) +
  scale_color_manual(values = road_colors) +
  labs(color = 'Road Type') + 
...
```{r}
state_boundary_US <- 
st_read(here("data", "NEON-DS-Site-Layout-Files/US-Boundary-Layers/US-State-Boundaries-Census-2014.shp"))
```
...
```{r}
ggplot(data = state_boundary_US) +
  geom_sf( aes(color = region), size = 1) +
...
newplot_locations_HARV <-
  read.csv(here("data","NEON-DS-Site-Layout-Files/HARV/HARV_2NewPhenPlots.csv"))
...
st_write(plot_locations_sp_HARV,
         here("data_output","PlotLocations_HARV.shp"), driver = "ESRI Shapefile")
```
