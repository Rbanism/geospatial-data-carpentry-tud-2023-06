...
```{r}
GDALinfo(here("data","NEON-DS-Airborne-Remote-Sensing/HARV/DSM/HARV_DSMhill.tif"))
```
...
```{r}
ggplot() +
  geom_bar(data = DSM_HARV_df, aes(fct_elevation_cb))
```
...
```{r}
ggplot() +
  geom_raster(data = DSM_HARV_df , aes(x = x, y = y, fill = fct_elevation_cb)) + 
  coord_quickmap()
```
...
```{r}
GDALinfo(here("data","NEON-DS-Airborne-Remote-Sensing/HARV/DTM/HARV_dtmCrop.tif"))
```
...
```{r}
writeRaster(CHM_ov_HARV, here("fig_output","CHM_HARV.tiff"),
            format="GTiff",
            overwrite=TRUE,
            NAflag=-9999)
```
...
```{r}
RGB_band2_HARV <-  raster(here("data","NEON-DS-Airborne-Remote-Sensing/HARV/RGB_Imagery/HARV_RGB_Ortho.tif"), band = 2)
```
...
```{r}
RGB_stack_HARV <- stack(here("data","NEON-DS-Airborne-Remote-Sensing/HARV/RGB_Imagery/HARV_RGB_Ortho.tif"))
```
...
```{r}
GDALinfo(here("data","NEON-DS-Airborne-Remote-Sensing/HARV/RGB_Imagery/HARV_Ortho_wNA.tif"))
```

```{r}
HARV_NA <- stack(here("data","NEON-DS-Airborne-Remote-Sensing/HARV/RGB_Imagery/HARV_Ortho_wNA.tif"))
```
...
```{r}
GDALinfo(here("data","NEON-DS-Airborne-Remote-Sensing/HARV/RGB_Imagery/HARV_RGB_Ortho.tif"))
```
...
```{r}
NDVI_HARV_path <- here("data","NEON-DS-Landsat-NDVI/HARV/2011/NDVI")
...
NDVI_HARV_stack_df <- as.data.frame(NDVI_HARV_stack, xy = TRUE) %>%
    gather(key = "variable", value = "value", -x,-y) #reshape2::melt(id.vars = c('x','y'))
```
...
```{r}
har_met_daily <-
  read.csv(here("data","NEON-DS-Met-Time-Series/HARV/FisherTower-Met/hf001-06-daily-m.csv"))
...
```{r}
RGB_277 <- stack(here("data","NEON-DS-Landsat-NDVI/HARV/2011/RGB/277_HARV_landRGB.tif"))
RGB_277
```
...
```{r}
# Julian day 293
RGB_293 <- stack(here("data","NEON-DS-Landsat-NDVI/HARV/2011/RGB/293_HARV_landRGB.tif"))
RGB_293 <- RGB_293/255
...
```{r}
NDVI_path_SJER <- here("data","NEON-DS-Landsat-NDVI/SJER/2011/NDVI")
...
```
```{r}
write.csv(avg_NDVI_HARV_clean, file=here("data_output","meanNDVI_HARV_2011.csv"))
```
...
```{r}
write.csv(avg_NDVI_SJER_clean, file = here("data_output","meanNDVI_SJER_2011.csv"))
```
