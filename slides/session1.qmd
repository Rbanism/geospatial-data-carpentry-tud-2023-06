...
**Hint:** Use `arrange()` to sort data frame or find minimum and maximum using `max()`  and `min()` functions.
...
gapminder %>%
  ...
  filter(
    mean_lifeExp == min(mean_lifeExp) | mean_lifeExp == max(mean_lifeExp)
    )
