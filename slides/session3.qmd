...
1. Download the data from **[https://edu.nl/trr7n]( https://edu.nl/trr7n)**
2. Extract its content into the data folder of your RStudio project

## `raster` package for spatial raster data

![](tudlib-green.png){fig-align="center"}


## Challenge 1: `r emo::ji("clock")` **2 mins**

Use `GDALinfo()` to determine the following about the `tud-dsm-5m-hill.tif` file:
...
Use the `tud-dtm-5m.tif` and `tud-dtm-5m-hill.tif` files from the `data` directory to create a Digital Terrain Model map of the TU Delft area.
